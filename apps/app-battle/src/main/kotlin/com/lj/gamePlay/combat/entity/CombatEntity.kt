...
class CombatEntity:Entity() {
    ...
    override fun awake() {
        ...
        currentHealth.reset()
        combatContext = MasterEntity.getTypeChildren<CombatContext>()?.get(0) as CombatContext
    }

    inline fun <reified T> createCombatAction(): T? where T:CombatAction{
        val action = combatContext.getComponent<CombatActionManageComponent>()?.createAction<T>(this)
        return action
    }
